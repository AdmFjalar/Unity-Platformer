...
public class CameraController : MonoBehaviour
{
	...
	void Update()
	{
		if (m_Target && m_Target.GetComponent<PlayerController>().lives > 0)
		{
			float targetX = m_Target.position.x + m_XOffset + Mathf.RoundToInt(m_Target.GetComponent<Rigidbody2D>().velocity.x);
			float targetY = m_Target.position.y + m_YOffset + Mathf.RoundToInt(m_Target.GetComponent<Rigidbody2D>().velocity.y);
